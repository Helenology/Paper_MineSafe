...
class SSSR:
    def __init__(self, X, middle_name, s, epsilon1=10):
        ...
        self.rp = 3               # the number of feature representations
    ...
    def convert_back(self, A):
        ...
        for frame_idx in range(self.k):
            for s_idx in range(self.s):
                feature = 0
                for rp_idx in range(self.rp):
                    real_idx = s_idx * self.rp + rp_idx
                    feature += A[real_idx, frame_idx]
                feature /= self.rp
                outcome[frame_idx][labels == s_idx] = feature
        return outcome